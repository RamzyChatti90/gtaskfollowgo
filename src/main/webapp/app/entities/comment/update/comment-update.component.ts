...
export class CommentUpdateComponent implements OnInit {
  ...
  setFileData(event: Event, field: string, isImage: boolean): void {
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe({
      error: (err: FileLoadError) =>
        this.eventManager.broadcast(new EventWithContent<AlertError>('gtaskfollowgoApp.error', { ...err, key: `error.file.${err.key}` })),
    });
  }
  ...
  protected loadRelationshipsOptions(): void {
    ...
    this.appUserService
      ...
      .pipe(map((appUsers: IAppUser[]) => this.appUserService.addAppUserToCollectionIfMissing<IAppUser>(appUsers, this.comment?.author)))
