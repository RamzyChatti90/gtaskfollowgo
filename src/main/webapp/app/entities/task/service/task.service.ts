...
type RestOf<T extends ITask | NewTask> = Omit<T, 'dueDate' | 'createdAt' | 'updatedAt'> & {
  dueDate?: string | null;
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...
      updatedAt: task.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...
      updatedAt: restTask.updatedAt ? dayjs(restTask.updatedAt) : undefined,
    };
  }
